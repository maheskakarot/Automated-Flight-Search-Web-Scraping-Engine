...
class RyanAirBasket:
    ...
    def click_on_full_details_button(self, fare_type):
        ...
        expanded_icons = self.driver.find_elements_by_class_name(CLASS_NAMES['EXPANDED_DROPDOWN'])

        for expanded_icon in expanded_icons:
            expanded_icon.click()
            time.sleep(1.2)
    ...
    def scrape_return_trip_basket_items(self, fare_type):
        ...
        if len(items) == 2:
            ...
            departure_items_info, departure_discount_items_info, departure_tax_items_info, departure_fast_track_info = self.scrape_cart_items(
                departure_items, fare_type)
            arrival_items_info, arrival_discount_items_info, arrival_tax_items_info, arrival_fast_track_info = self.scrape_cart_items(
    ...
    def scrape_basket_items(self, fare_type):
        ...
        if len(items) == 2:
            ...
            departure_basket_data = self.scrape_cart_items(departure_items, fare_type)
            arrival_basket_data = self.scrape_cart_items(arrival_items, fare_type)
        ...
        basket_full_data = {
            'departure_items_info': departure_basket_data['basket_items_info'],
            'departure_discount_items_info': departure_basket_data['discount_items_info'],
            'departure_tax_items_info': departure_basket_data['tax_items_info'],
            'departure_fast_track_info': departure_basket_data['fast_track_info'],
            'departure_additional_seat_info': departure_basket_data['additional_seat_info'],
            'departure_paid_items_data': departure_basket_data['paid_items_data'],
            'departure_extras': departure_basket_data['extras'],
        }

        if arrival_basket_data:
            basket_full_data.update({
                'arrival_items_info': arrival_basket_data['basket_items_info'],
                'arrival_discount_items_info': arrival_basket_data['discount_items_info'],
                'arrival_tax_items_info': arrival_basket_data['tax_items_info'],
                'arrival_fast_track_info': arrival_basket_data['fast_track_info'],
                'arrival_additional_seat_info': arrival_basket_data['additional_seat_info'],
                'arrival_paid_items_data': arrival_basket_data['paid_items_data'],
                'arrival_extras': arrival_basket_data['extras'],
            })

        return basket_full_data
    ...
    def scrape_cart_items(self, cart_items, fare_type):
        ...
        for item in items:
            ...
            if 'discount' in item_list[0].lower() or 'saved' in item_list[0].lower():
            ...
            elif "fast track" in item_list[0].lower():
                fast_track_item_info = {}
                price_info = {}

                fast_track_count = int(item_list[0].split(" x ")[0])

                if len(item_list) == 3:
                    price_info = {
                        "currency": item_list[1],
                        "value": float("{}".format(item_list[2]).replace(",", "")),
                    }

                elif len(item_list) == 5:
                    price_info = {
                        "currency": item_list[1],
                        "value": float("{}{}{}".format(item_list[2], item_list[3], item_list[4]).replace(",", "")),
                    }

                fast_track_item_info["count"] = fast_track_count
                fast_track_item_info["price_info"] = price_info
                fast_track_info = copy.deepcopy(fast_track_item_info)

            elif "additional seat cost" in item_list[0].lower():
                additional_seat_item_info = {}
                price_info = {}

                additional_seat_count = int(item_list[0].split(" x ")[0])
                ...
                additional_seat_item_info["count"] = additional_seat_count
                additional_seat_item_info["price_info"] = price_info
                additional_seat_info = copy.deepcopy(additional_seat_item_info)

            elif "bag" in item_list[0].lower() or "equipment" in item_list[0].lower():
                extra_item_info = {}
                price_info = {}

                item_count = int(item_list[0].split(" x ")[0])
                item_name = item_list[0].split(" x ")[1]
                ...
                extra_item_info["name"] = item_name
                extra_item_info["count"] = item_count
                extra_item_info["price_info"] = price_info
                extra_item_info = copy.deepcopy(extra_item_info)
                extras.append(extra_item_info)

            elif "reserved seat" in item_list[0].lower():
                paid_item_info = {}
                price_info = {}
                ...
                paid_item_info["name"] = item_name
                paid_item_info["count"] = fast_track_count
                paid_item_info["price_info"] = price_info
                paid_item_info = copy.deepcopy(paid_item_info)
                paid_items_data.append(paid_item_info)
        ...
        basket_data = {
            'basket_items_info': basket_items_info,
            'discount_items_info': discount_items_info,
            'tax_items_info': tax_items_info,
            'fast_track_info': fast_track_info,
            'additional_seat_info': additional_seat_info,
            'paid_items_data': paid_items_data,
            'extras': extras
        }
