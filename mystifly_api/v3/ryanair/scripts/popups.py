...
class HandlePopups():
    ...
    def dismiss_crowd_popup(self, fare_type):
        ...
        if fare_type != "FPF":
            try:
                self.driver.find_element_by_class_name(CLASS_NAMES['DISMISS_CROWD_POPUP_BTN']).click()
            except Exception as ex:
    ...
    def dismiss_family_seating_popup(self):
        ...
        for i in range(5):
            try:
                family_seating_cta_btn = self.driver.find_element_by_class_name(CLASS_NAMES['FAMILY_SEATING_CTA_BTN'])
