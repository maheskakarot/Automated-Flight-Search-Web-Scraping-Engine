...
import datetime
import logging
from v4.ryanair.helpers.search import search_api_response, get_fare_values_v4
from v4.ryanair.helpers.basket import create_booking, create_basket, create_multiple_bookings_v2
...
class SearchFlightAutomation:
            ...
            if len(five_dates_data) == 5:
                ...
                if departure_flights:
                    ...
                    for departure_flight in departure_flights:
                        ...
                        fare_breakdown_request_data = {
                            'basketId': basket_id,
                            'departureFareKey': departure_flight['regularFare']['fareKey'],
                            'departureFlightKey': departure_flight['flightKey'],
                            'arrivalFareKey': None,
                            'arrivalFlightKey': None
                        }
                        fare_breakdown_requests_data.append(fare_breakdown_request_data)
                        results_count += 1

                    fare_breakdowns_data = self.get_fare_breakdowns_data(fare_breakdown_requests_data)
                    ...
                    for departure_flight in departure_flights:
                        ...
                        create_booking_response = fare_breakdowns_data.get(departureFlightKey, None)

                        if not create_booking_response:
                            continue
    ...
    def return_trip_flights(self, search_url):
        ...
        if departure_flight_data and arrival_flight_data:
            ...
            if len(dept_five_dates_data) == 5 and len(arvl_five_dates_data) == 5:
                ...
                # Fare breakdown data of target combinations
                fare_breakdowns_data = self.get_fare_breakdowns_data(fare_breakdown_requests_data)
                ...
                for departure_flight in departure_flights:
                    ...
                    for arrival_flight in arrival_flights:
                        ...
                        flight_key = "{}-{}".format(departureFlightKey, arrivalFlightKey)

                        create_booking_response = fare_breakdowns_data.get(flight_key, None)
    ...
    def test_value_fare_data(self, constant_fields_data, create_booking_response, is_return):
        data = {
    ...
    def get_fare_breakdowns_data(self, flights_payload_data):
        requests_data = []

        for flight_payload_data in flights_payload_data:

            requests_data.append({
                'basketId': flight_payload_data['basketId'],
                'adults': self.adults,
                'children': self.children,
                'infants': self.infants,
                'departureFareKey': flight_payload_data['departureFareKey'],
                'departureFlightKey': flight_payload_data['departureFlightKey'],
                'arrivalFareKey': flight_payload_data['arrivalFareKey'],
                'arrivalFlightKey': flight_payload_data['arrivalFlightKey'],
            })

        fare_breakdowns_data = create_multiple_bookings_v2(requests_data)

        return fare_breakdowns_data
