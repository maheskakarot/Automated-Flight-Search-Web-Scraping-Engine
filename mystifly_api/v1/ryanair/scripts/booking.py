...
from .passengers_detail import PassengersDetail
...
class Booking:
    ...
    def initiate_booking(self, is_driver_reused, subscriber, user_last_screen):
        ...
        if len(result_id_items) == 4:
            ...
            if not is_driver_reused:
                # Added sleep time to avoid scraping detection
                time.sleep(1)
                self.reprice_validation_obj.click_on_return_trip_target_flight_card(departure_flight_number, arrival_flight_number)
    ...
    def one_way_seat_selection(self, subscriber, fare_type):
        ...
        is_selected, warning_title, warning_body = self.seat_selection_obj.check_seat_availability(self.request_data["PassengersSeatsInfo"])
        if is_selected:
    ...
    def return_trip_seat_selection(self, fare_type):
        ...
        if self.request_data["SeatSelectionFor"] == "Departure Flight":
            ...
            seat_map = self.seat_selection_obj.prepare_seat_map()
    ...
    def baggages_selection(self, is_driver_reused, fare_type, is_return_trip):
        response_data = {'CartItems': {}}
